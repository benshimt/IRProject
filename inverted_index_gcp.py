...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes,directory):
        b = []
...
class InvertedIndex:  
    ...
    def write_a_posting_list_sort(self, w, writer, sort=False):
        # sort the posting list by doc_id
        pl = self.posting_list[w]
        if sort:
    ...
    def __getstate__(self):
        ...
        return state
    def read_posting_list(self, w, directory):
    ...
    def posting_lists_iter(self,directory):
        ...
        with closing(MultiFileReader()) as reader:
            for w, locs in self.posting_locs.items():
                b = reader.read(locs[0], self.df[w] * TUPLE_SIZE,directory)
                posting_list = []
                for i in range(self.df[w]):
                    doc_id = int.from_bytes(b[i*TUPLE_SIZE:i*TUPLE_SIZE+4], 'big')
                    tf = int.from_bytes(b[i*TUPLE_SIZE+4:(i+1)*TUPLE_SIZE], 'big')
    ...
    @staticmethod
    def write_a_posting_list_anchor(b_w_pl, bucket_name):
        posting_locs = defaultdict(list)
        bucket_id, list_w_pl = b_w_pl
        ...
        with closing(MultiFileWriter('.', bucket_id, bucket_name)) as writer:
            for w, pl in list_w_pl:
                # convert to bytes
                b = b''.join([(doc_id << 16).to_bytes(TUPLE_SIZE, 'big')
                              for doc_id in pl])
                # write to file(s)
                locs = writer.write(b)
                # save file locations to index
